...
### 2.0 The problem the corpus did not anticipate: there is no room

Every corpus test for what belongs in a model terminates in a human oracle — the business expert
who tells you immediately, who has a term for it in their meetings, who makes a puzzled face.
**FnEmail has no such person.** Our business expert is RFC 5321, a document. It cannot be puzzled.

This is a real methodological gap, not a detail. It is why we cannot simply adopt §1.4 and stop,
and why rules 2.3 and 2.4 exist: they are **written proxies for the absent human**. It is also why
`event-model.md`'s existing caveat — *"No source in the corpus applies Event Modeling to a protocol
server"* — extends further than that section claims. It applies to altitude too.
