...
| **ReversePathAllowed names the step 6 view** — a view names the server's response to its occasioning event, the pattern the other rendered views already followed; the naming split the slice; the `RecipientAccepted`-occasioned half was named `RecipientConfirmed` later the same day (2026-08-09) | `paths/EXPLORE-declaration-vs-status.md` |
| **DataRequested names the DATA request; H1 closes** — the two-kinds lens's first application: §4.3.2's `I:` rules out decision names, the status-register incumbent `DataPhaseEntered` fell, and the capture verb carries the protocol's own provenance (§4.2.2 *"Requested mail action"*); both H1 halves closed (2026-08-09) | `paths/EXPLORE-declaration-vs-status.md` |
